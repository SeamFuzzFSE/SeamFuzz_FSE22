...
void update_tmp_succ_group_prob_info(afl_state_t *afl, u32 id_num) {
  ...
  universe = 0;
  for (int i = 0; i < sp_num; i++) univ_prob[i] = 0;
...
void update_prob_using_group(afl_state_t *afl, s32 id_num) {
...
/* returns 1 when any rare path is covered.
   returns 2 when all covered paths are common ones.
   o.w. retunrs 0.
*/
u8 check_hit_table (afl_state_t *afl) {
  ...
  for (int i = 0; i < afl->working_index; i++) {
...
static void sorting_hit_table(afl_state_t *afl) {
  u32 limit, tmp_index;
  int i, j;
  ...
  for (u32 k = 0; k < (MAP_SIZE << 3); k++) {
    ...
    if ((signed) tmp_index == afl->working_index) FATAL("Invalid index in sorting_hit_table");
  ...
  while(tmp_hit_table[i].hit_cnt <= limit) {
    ...
    //afl->checking_table[(tmp_hit_table[i].index << 3) + tmp_hit_table[i].bin_index] = 1;
    i--;
    k++;
  }

  // update common path info
  //limit = tmp_hit_table[working_index / 10].hit_cnt;
  ...
  while(tmp_hit_table[i].hit_cnt >= limit) {
    ...
    //afl->checking_table[(tmp_hit_table[i].index << 3) + tmp_hit_table[i].bin_index] = 2;
    i++;
  }
...
*/
static double calculate_rareness(afl_state_t *afl) {  
  ...
  for (int i = 0; i < afl->current_working_index; i++) {
    ...
    if (tmp_index != MAP_SIZE) {
      tmp_byte = afl->current_file_trace[tmp_index] & afl->group_covered_paths[tmp_index];
    ...
    if (unlikely(tmp)) {
      for (j = 0; j < 8; j++) {
        ...
        if(tmp_bit == 0) continue;
          ...
          if (afl->real_hit_table[(i << 3) + j].hit_cnt < 512) afl->real_hit_table[(i << 3) + j].hit_cnt++;
        }
      }
    }
  }
...
static void read_covered_path(afl_state_t* afl, u8* fname) {
...
static void read_group_covered_path(afl_state_t* afl, u8* fname) {
  s32 fd = open(fname, O_RDONLY);
...
void init_group_path(afl_state_t* afl) {
  ...
  u8 *common_fn = alloc_printf("%s/%d", afl->group_common_path_dir, afl->prob_group_cnt);  

  fd = open(common_fn, O_WRONLY | O_CREAT | O_TRUNC, 0600);

  if (fd < 0) PFATAL("Unable to open '%s'", common_fn);

  ck_write(fd, afl->src_file_covered_path, MAP_SIZE, common_fn);
  ...
  for (u32 i = 0; i < MAP_SIZE; i++) {
    if (afl->src_file_covered_path[i] != 0) {
      for (u8 j = 0; j < 8; j++) {        
        u8 tmp_bit = afl->src_file_covered_path[i] & (1 << (7 - j));

        if (tmp_bit != 0) afl->group_common_sim[afl->prob_group_cnt]++;
      }
    }
...
*/
void write_group_path(afl_state_t* afl) {
  s32 fd;

  u8 *total_fn = alloc_printf("%s/%d", afl->group_covered_path_dir, afl->prob_group_id);
  read_group_covered_path(afl, total_fn);     //group_covered_paths

  u8 *common_fn = alloc_printf("%s/%d", afl->group_common_path_dir, afl->prob_group_id);
  read_covered_path(afl, common_fn);        //src_file_covered_path

  afl->group_common_sim[afl->prob_group_id] = 0;

  for (u32 i = 0; i < MAP_SIZE; i++) {
    afl->src_file_covered_path[i] = afl->src_file_covered_path[i] & afl->current_file_trace[i]; // for common paths.
    
    afl->group_covered_paths[i] = afl->group_covered_paths[i] | afl->current_file_trace[i];     // for total paths

    if (afl->src_file_covered_path[i] != 0) {
      for (u8 j = 0; j < 8; j++) {
        u8 tmp_bit = afl->src_file_covered_path[i] & (1 << (7 - j));

        if (tmp_bit != 0) afl->group_common_sim[afl->prob_group_id]++;
      }
    }
...
u32 select_starting_point(afl_state_t *afl, u32 limit) {
  ...
  PFATAL("Invalid statring point is chosne");
  return 100;
}
...
u32 select_operator (afl_state_t *afl, u32 limit) {
  ...
  PFATAL("Invalid operator is chosne");
  return 100;
}

void print_seed_swarm_indv(afl_state_t *afl, u32 file_id, u32 group_id, double syn_sim, double sem_sim, double rare) {
  u8 *prob_dir_file = alloc_printf("%s/indv_seed", afl->seam_prob_dir);

  if (access(prob_dir_file, 0) == -1)
    if (mkdir(prob_dir_file, 0700)) PFATAL("Unable to create '%s'", prob_dir_file);

  u8 *prob_dir_file_tmp = alloc_printf("%s/%d", prob_dir_file, group_id);

  FILE *seed_file = fopen(prob_dir_file_tmp, "a+");

  fprintf(seed_file, "%d, %d, %d, %1.5f, %1.5f, %1.5f, %1.5f\n", file_id, afl->current_working_int_id, afl->bt_seed, syn_sim, sem_sim, rare, afl->rareness_score);

  fclose(seed_file);
  ck_free(prob_dir_file_tmp);
...
void print_seed_in_swarm_in_file(afl_state_t *afl) {
  ...
  FILE *seed_file = fopen(prob_dir_file, "a+");

  fprintf(seed_file, "%d, %1.5f, %d, %d, %1.5f, %1.5f\n", afl->is_splice, afl->gamma_val, afl->bt_seed, afl->current_working_int_id, afl->syn_sim, afl->sem_sim);
...
void print_seed_similarity(afl_state_t *afl, u8* in_buf,u32 len) {
  struct dirent *de;  // Pointer for directory entry
  double syntactic_sim, semantic_sim, checking_sim1, checking_sim2, checking_sim3, checking_sim4, semantic_sim4;
  s32 fd;
  u32 i, testing_len, flag;
  DIR *dr = opendir(afl->covered_path_dir);
  
  if (dr == NULL)  PFATAL("Unable to find covered files");
  
  u8 *afl_queue_path = alloc_printf("%s/queue/", afl->out_dir);

  u8 *sim_dir = alloc_printf("%s/similarity", afl->seam_prob_dir);
  //u8 *sim_dir_file = alloc_printf("%s/similarity/%d", afl->seam_prob_dir, afl->current_working_int_id);
  u8 *sim_dir_file = alloc_printf("%s/sim_result", sim_dir);

  if (access(sim_dir, 0) == -1)
    if (mkdir(sim_dir, 0700)) PFATAL("Unable to create '%s'", sim_dir);

    
  FILE *seed_file = fopen(sim_dir_file, "a+");
  //fprintf(seed_file, "Seed File, div by current bit, div by checking bit\n");

  while ((de = readdir(dr)) != NULL) {
    if (de->d_type != DT_REG) continue;

    struct dirent *de_queue;
  
    DIR *dr_queue = opendir(afl_queue_path);

    if (dr_queue == NULL) PFATAL("Unable to find queue directory");

    while ((de_queue = readdir(dr_queue)) != NULL) {
      if (de_queue->d_type != DT_REG) continue;
      
      u8 *tmp_fname = alloc_printf("%s", de_queue->d_name);
      u8 *tmp_fn_id = strtok(tmp_fname, ":");
      tmp_fn_id = strtok(NULL, ",");

      if (atoi(tmp_fn_id) == atoi(de->d_name)) {
        u8 *queue_file_name = alloc_printf("%s/%s", afl_queue_path, de_queue->d_name);
        FILE *file_pointer = fopen(queue_file_name, "r");
        fseek(file_pointer, 0, SEEK_END);
        testing_len = ftell(file_pointer);

        fclose(file_pointer);
        
        fd = open(queue_file_name, O_RDONLY);

        if (fd < 0) PFATAL("Unable to open '%s' in print_seed_similiarity function", queue_file_name);
      
        u8 *src_file_buf = mmap(0, testing_len, PROT_READ | PROT_WRITE, MAP_PRIVATE, fd, 0);

        if (src_file_buf == MAP_FAILED) PFATAL("Unable to mmap '%s' - in print seed similarity function", queue_file_name);

        close(fd);

        if (len > testing_len) {          // When the seed input is larger than representative seed input
          for (i = 0; i < testing_len; i++) {
            u8 cur = in_buf[i];
            u8 cur_src = src_file_buf[i];

            if (cur != cur_src) {
              for (u8 j = 0; j < 8; j++) {
                u8 tmp_bit = cur & (1 << (7 - j));
                u8 tmp_src_bit = cur_src & (1 << (7 -j));
                if (tmp_bit != 0 && tmp_src_bit != 0) syntactic_sim += 1.0;
              }
            }
          }

          syntactic_sim = syntactic_sim / (double)(len * 8);
          flag = 1;

        } else if (len <= testing_len) {
          for (i = 0; i < len; i++) {
            u8 cur = in_buf[i];
            u8 cur_src = src_file_buf[i];
          ...
          syntactic_sim = syntactic_sim / (double)(testing_len * 8); 
          flag = 2;
        }
                
        if (munmap(src_file_buf, testing_len) != 0) PFATAL("Unable to deallocate src_file_buf in calculate_distance function");
        ck_free(tmp_fname);
        ck_free(queue_file_name);
        break;
      } else {
        ck_free(tmp_fname);
        continue;
    ...
    u8 *tmp_covered_path = alloc_printf("%s/%s", afl->covered_path_dir, de->d_name);

    read_covered_path(afl, tmp_covered_path);

    for (i = 0; i < MAP_SIZE; i++) {
      u8 tmp1 = afl->current_file_trace[i];
      u8 tmp2 = afl->src_file_covered_path[i];
      if (tmp1 != 0 && tmp2 != 0) {
        for (u8 j = 0; j < 8; j++) {
          u8 tmp1_bit = tmp1 & (1 << (7 - j));
          u8 tmp2_bit = tmp2 & (1 << (7 - j));

          if (tmp1_bit != 0 && tmp2_bit != 0) {
            semantic_sim += 1.0;
            semantic_sim4 += (1.0 / (double) afl->real_hit_table[(i << 3) + j].hit_cnt);
      ...
      if (tmp1 != 0) {
        for (u8 j = 0; j < 8; j++) {
          u8 tmp_bit = tmp1 & (1 << (7 - j));
          if (tmp_bit != 0) checking_sim1 += 1.0;
        }
      }

      if (tmp2 != 0) {
        for (u8 j = 0; j < 8; j++) {
          u8 tmp_bit = tmp2 & (1 << (7 - j));
          if (tmp_bit != 0) checking_sim2 += 1.0;
        }
      }
      ...
      if (tmp1 != 0 || tmp2 != 0) {
        for (u8 j = 0; j < 8; j++) {
          ...
          if (tmp1_bit != 0 || tmp2_bit != 0) {
            checking_sim3 += 1.0;
            checking_sim4 += (1.0 / (double) afl->real_hit_table[(i << 3) + j].hit_cnt);
          }
        }
      }
    } 

    checking_sim1 = semantic_sim / checking_sim1;
    checking_sim2 = semantic_sim / checking_sim2;
    checking_sim3 = semantic_sim / checking_sim3;
    checking_sim4 = semantic_sim4 / checking_sim4;

    fprintf(seed_file, "%d, %s, %d, %1.5f, %1.5f, %1.5f, %1.5f, %1.5f\n", afl->current_working_int_id, de->d_name, flag, syntactic_sim, checking_sim1, checking_sim2, checking_sim3, checking_sim4);
    ...
    ck_free(tmp_covered_path);
  }

  closedir(dr);    
  
  fclose(seed_file);
  ck_free(sim_dir_file);
  ck_free(sim_dir);
  ck_free(afl_queue_path); 
...
void print_updated_probability(afl_state_t *afl, u32 file_id) {
  ...
  for (int i = 0; i < 61; i++) {
    fprintf(op_file, "%d, ", afl->op_prob[i]);

    if (i < sp_num) {
      fprintf(sp_file, "%d, ", afl->sp_prob[i]);
    } 
  } 
...
void update_candidate_seed(afl_state_t *afl, struct cluster_table *update_group, u32 in_buf_len) {
  double current_rare_score = 0.0;
  double cand_rare_score = 0.0;
  double rare_hit_cnt = 0.0;
  
  read_covered_path(afl, update_group->file_buf);
    
  for (int i = 0; i < afl->current_working_index; i++) {
    u32 tmp_index = afl->rare_hit_table[i].index;

    if (tmp_index != MAP_SIZE) {
      u8 group_tmp = afl->src_file_covered_path[tmp_index];
      u8 current_tmp = afl->current_file_trace[tmp_index];
...
// returns 2 when the given seed input is the representative seed input (making no new group)
// returns 1 when no representative seed input is similar to the given seed input. (make new group)
// returns the representative seed input id when it is similar to the given seed input.
// if it is very similar to the representative seed input, do not make new group.
// if it is similar to the representative seed input not enough, make new group.
static u32 calculate_distance(afl_state_t *afl, u8* in_buf, u32 in_buf_len) {
  struct cluster_table *checking_group = afl->file_table_ori;
  struct cluster_table *update_group = afl->file_table_ori;
  double semantic_sim, syntactic_sim, final_similarity;
  double rareness_score = 0.0;
  ...
  while (checking_group) {
    if (afl->group_delete[checking_group->group_id] == 2) {
    ...
    u8* src_file_buf;
    syntactic_sim = 0.0;
    testing_len = checking_group->len;
    
    /* calculate syntactic similarities. */ 
    if (afl->use_seamfuzz != 2) {
      /* Map the test case into memory. */

      fd = open(checking_group->file_buf, O_RDONLY);

      if (fd < 0) PFATAL("Unable to open '%s' in calculate_distance function", checking_group->file_buf);

      src_file_buf = mmap(0, testing_len, PROT_READ | PROT_WRITE, MAP_PRIVATE, fd, 0);

      if (src_file_buf == MAP_FAILED) PFATAL("Unable to mmap '%s' - in calculate_distance function", checking_group->file_buf);

      close(fd);

      if (in_buf_len > testing_len) {
        for (i = 0; i < testing_len; i++) {
          u8 tmp_byte = in_buf[i] & src_file_buf[i];

          if (tmp_byte != 0) {
            for (u8 j = 0; j < 8; j++) {
              u8 tmp_bit = tmp_byte & (1 << (7 - j));

              if (tmp_bit != 0) syntactic_sim += 1.0;
            }
          }
        }
      ...
      munmap(src_file_buf, testing_len);
    }
    
    /* calculate semantic similarities */
    
    read_covered_path(afl, checking_group->group_common_path);

    semantic_sim = 0.0;
    rareness_score = 0.0;
    
    for (i = 0; i < MAP_SIZE; i++) {
      u8 tmp_byte = afl->current_file_trace[i] & afl->src_file_covered_path[i];
      ...
      for (u8 j = 0; j < 8; j++) {
        ...
        if (tmp_bit != 0) semantic_sim += 1.0;
      }

    }
    
    read_group_covered_path(afl, checking_group->group_covered_path);    
    rareness_score = calculate_rareness(afl);  

    semantic_sim = semantic_sim / (double) afl->group_common_sim[checking_group->group_id]; 

    final_similarity = afl->use_seamfuzz == 2 ? semantic_sim : syntactic_sim * afl->alpha_val + semantic_sim * afl->beta_val;
    //final_similarity = final_similarity + rareness_score;
    //final_similarity += rareness_score(afl, checking_group);
    final_similarity = final_similarity * afl->sim_rate + rareness_score * afl->rareness_rate;
    ...
    if (afl->use_seamfuzz_debuging == 1) print_seed_swarm_indv(afl, checking_group->file_id, checking_group->group_id, syntactic_sim, semantic_sim, rareness_score);
  ...
  if (max_sim_check < afl->gamma_val) {
    ...
    u8 *cand_file_path = alloc_printf("%s/%d", afl->covered_path_dir, update_group->file_id);
    
    read_covered_path(afl, cand_file_path);
...
void update_success_info(afl_state_t *afl) {
  ...
  for (int i = 0; i < afl->stage_cur_val; i++) {
    afl->success_op_prob[file_id][afl->op_trace[i]]++;
    afl->success_sp_prob[file_id][afl->sp_trace[i]]++;
  }

  afl->success_update_flag = 1;
  //afl->group_updated
...
void update_fail_info(afl_state_t *afl) {
  ...
  for (int i = 0; i < afl->stage_cur_val; i++) {
    afl->fail_op_prob[file_id][afl->op_trace[i]]++;
    afl->fail_sp_prob[file_id][afl->sp_trace[i]]++;
  } 

  afl->fail_update_flag = 1;
...
u8 fuzz_one_seam(afl_state_t *afl) {
  ...
  afl->success_update_flag = 0;
  afl->fail_update_flag = 0;
  ...
  //if (afl->use_seamfuzz_debuging == 1) print_seed_similarity(afl, in_buf, len);
...
#define MAX_HAVOC_ENTRY 59                                      /* 55 to 60 */
  ...
  if (afl->is_splice == 0) {
    ...
    // sorting every 30 min or 1000 new seeds are added into queue.
    //if (afl->sorting_start_time == 0 || get_cur_time() - afl->sorting_start_time > 30 * 60 * 1000 || afl->cnt_to_sorting > 1000) {
      if (afl->cnt_to_sorting != 0) {
        sorting_hit_table(afl);

        afl->cnt_to_sorting = 0;
      }
      //afl->sorting_start_time = get_cur_time();
    //}
      //sorting_hit_table(afl);
  
      s32 current_fd;
      memset(afl->current_file_trace, 0, MAP_SIZE);
      ...
      ck_read(current_fd, afl->current_file_trace, MAP_SIZE, current_fname_id);
      ...
      afl->prob_group_id = calculate_distance(afl, out_buf, len);
      //afl->prob_group_id = UINT_MAX;

      if (afl->prob_group_id == UINT_MAX) afl->prob_group_id = afl->prob_group_cnt;
      else {
        use_existing_prob2 = 1;
        afl->grouping_cnt++;

        write_group_path(afl);

        update_prob_using_group(afl, afl->prob_group_id);
  ...
  for (afl->stage_cur = 0; afl->stage_cur < afl->stage_max; ++afl->stage_cur) {
    ...
    memset(afl->op_trace, 30, 128);
    memset(afl->sp_trace, sp_num, 128);

    afl->stage_cur_val = use_stacking;

    u32 end_point, starting_dist, current_place, starting_point;
    u32 tmp_dist;    
      ...
      switch (r) {
        ...
        case 0 ... 3: {
          ...
          if (temp_len < 5) {
            ...
            if (starting_point > end_point - 1) starting_point = end_point - 1;

            current_place = starting_point + rand_below(afl, starting_dist);
          }
          
          afl->sp_trace[i] = tmp_dist;
        ...
        case 4 ... 7: {
          ...
          starting_dist = (temp_len - 1) / sp_num;
          tmp_dist = select_starting_point(afl, sp_num);
          starting_point = starting_dist * tmp_dist;

          if (starting_point > temp_len - 1) starting_point = temp_len - 1;
        ...
        case 8 ... 9: {
          ...
          starting_dist = (temp_len - 2) / sp_num;
          tmp_dist = select_starting_point(afl, sp_num);
          starting_point = starting_dist * tmp_dist;

          if (starting_point > temp_len - 2) starting_point = temp_len - 2;
        ...
        case 12 ... 13: {
          ...
          starting_dist = (temp_len - 4) / sp_num;
          tmp_dist = select_starting_point(afl, sp_num);
          starting_point = starting_dist * tmp_dist;

          if (starting_point > temp_len - 4) starting_point = temp_len - 4;
        ...
        case 16 ... 19: {
          ...
          if (temp_len < 11) {
            ...
            starting_dist = (end_point - 1) / sp_num; 
            tmp_dist = select_starting_point(afl, sp_num);
            starting_point = starting_dist * tmp_dist;
        ...
        case 24 ... 25: {
          ...
          end_point = temp_len;
          if (temp_len < 12) {
            current_place = rand_below(afl, end_point - 1);
          } else {
            starting_dist = (end_point - 2) / sp_num; 
            tmp_dist = select_starting_point(afl, sp_num);
            starting_point = starting_dist * tmp_dist;

            if (starting_point > end_point - 2) starting_point = end_point - 2;
        ...
        case 32 ... 33: {
          ...
          end_point = temp_len;
          if (temp_len < 14) {
            current_place = rand_below(afl, end_point - 3);
          } else {
            starting_dist = (end_point - 4) / sp_num; 
            tmp_dist = select_starting_point(afl, sp_num);
            starting_point = starting_dist * tmp_dist;

            if (starting_point > end_point - 4) starting_point = end_point - 4;
        ...
        case 44 ... 46: {
          ...
          if (temp_len + HAVOC_BLK_XL < MAX_FILE) {
            ...
            if (temp_len < 11) {
              current_place = rand_below(afl, temp_len);
              tmp_dist = sp_num;
            } else {
              starting_dist = (temp_len - 1) / 10; 
              tmp_dist = select_starting_point(afl, sp_num);
              starting_point = starting_dist * tmp_dist;
        ...
        case 48 ... 50: {
          ...
          if (temp_len < 10 + copy_len) {
            current_place = rand_below(afl, temp_len - copy_len + 1);
            tmp_dist = sp_num;
          } else {
            starting_dist = (temp_len - copy_len) / 10; 
            tmp_dist = select_starting_point(afl, sp_num);
            starting_point = starting_dist * tmp_dist;
        ...
        case 52 ... MAX_HAVOC_ENTRY: {
          ...
          if (temp_len < 10 + del_len) {
            current_place = rand_below(afl, temp_len - del_len + 1);
            tmp_dist = sp_num;
          } else {
            starting_dist = (temp_len - del_len) / 10;    
            tmp_dist = select_starting_point(afl, sp_num);         
            starting_point = starting_dist * tmp_dist;   

            if (starting_point > temp_len - del_len) starting_point = temp_len - del_len;
        ...
        default:
          ...
          if (afl->extras_cnt) {
            ...
            if (r < 2) {
              ...
              if (temp_len < 10) {
                current_place = rand_below(afl, temp_len - extra_len + 1);
                tmp_dist = sp_num;
              } else {
                starting_dist = (temp_len - extra_len + 1) / 10; 
                tmp_dist = select_starting_point(afl, sp_num);           // 0 ~ 9
                starting_point = starting_dist * tmp_dist;

                if (starting_point > temp_len) starting_point = temp_len;
              ...
              if (temp_len < 10) {
                current_place = rand_below(afl, temp_len + 1);
                tmp_dist = sp_num;
              } else {
                starting_dist = temp_len / 10; 
                tmp_dist = select_starting_point(afl, sp_num);           // 0 ~ 9
                starting_point = starting_dist * tmp_dist;
  ...
  if (afl->is_splice == 0) {
    if (afl->group_delete[afl->prob_group_id] != 2 && afl->exploit[afl->prob_group_id] > 5) {
      if (afl->group_perf[afl->prob_group_id] - afl->explore[afl->prob_group_id] == 0) {
        afl->group_delete[afl->prob_group_id] = 2;
        afl->delete_cnt++;
      } else {
        afl->explore[afl->prob_group_id] = afl->group_perf[afl->prob_group_id];
        //afl->group_perf[afl->prob_group_id] = 0;
        afl->exploit[afl->prob_group_id] = 0;
      }
    }

    if (afl->success_update_flag == 1 || afl->fail_update_flag == 1) {

      if (afl->file_table == NULL) {
        ...
        //afl->file_table->covered_path = alloc_printf("%s/%d", afl->covered_path_dir, afl->current_working_int_id);
        afl->file_table->group_covered_path = alloc_printf("%s/%d", afl->group_covered_path_dir, afl->prob_group_cnt);
        afl->file_table->group_common_path = alloc_printf("%s/%d", afl->group_common_path_dir, afl->prob_group_cnt);

        init_group_path(afl);

        afl->file_table_ori = afl->file_table;

        afl->seed_group_id[afl->current_working_int_id] = afl->prob_group_cnt;
        afl->prob_group_cnt++;
        ...
        //tmp_file_table->covered_path = alloc_printf("%s/%d", afl->covered_path_dir, afl->current_working_int_id);
        tmp_file_table->group_covered_path = alloc_printf("%s/%d", afl->group_covered_path_dir, afl->prob_group_cnt);
        tmp_file_table->group_common_path = alloc_printf("%s/%d", afl->group_common_path_dir, afl->prob_group_cnt);
...
u8 fuzz_one(afl_state_t *afl) {
  ...
  if (afl->use_seamfuzz != 0) { key_val_lv_1 = fuzz_one_seam(afl); }
