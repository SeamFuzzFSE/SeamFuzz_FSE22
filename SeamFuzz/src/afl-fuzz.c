...
int main(int argc, char **argv_orig, char **envp) {
  ...
  while ((opt = getopt(
              argc, argv,
              "+b:B:c:CdDe:E:hi:I:f:F:l:L:m:M:nNOo:p:RQs:S:t:T:UV:Wx:ZlK:k")) > 0) {
    ...
    switch (opt) {
      ...
      case 'K':      /* 2: only use semantic similarity*/
        ...
        afl->sim_rate = 1.0;
        afl->rareness_rate = 1.0;
        
        afl->cnt_to_sorting = 1;

        afl->prob_group_cnt = 1;
        afl->prob_group_id = 1;

        afl->sorting_start_time = 1;
        afl->explore_prob = 30;
