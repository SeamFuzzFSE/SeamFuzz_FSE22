...
#define FOREIGN_SYNCS_MAX 32U
  ...
  struct hit_table rare_hit_table[(MAP_SIZE << 3)];
  struct hit_table common_hit_table[(MAP_SIZE << 3)];
  
  double gamma_val;
  double alpha_val;
  double beta_val;
  double sim_rate;
  double rareness_rate;
  ...
  u8 use_existing_prob;      // 0 -> do not use updated prob. 1 -> 
  u32 explore_prob;          // initialize to 30 when seamfuzz starts... 

  u32 op_prob[61];
  u32 sp_prob[sp_num];

  u32 success_update_flag;
  u32 fail_update_flag;

  /* probability for grouping option */
  u32 success_op_prob[MAP_SIZE][61];
  u32 success_sp_prob[MAP_SIZE][sp_num + 1];

  u32 fail_op_prob[MAP_SIZE][61];
  u32 fail_sp_prob[MAP_SIZE][sp_num + 1];

  u32 tmp_succ_op_prob[61];
  u32 tmp_succ_sp_prob[sp_num];

  u32 tmp_fail_op_prob[61];
  u32 tmp_fail_sp_prob[sp_num];

  u32 group_common_sim[MAP_SIZE];
  ...
  u8 current_file_trace[MAP_SIZE];

  //u32 table_exists[MAP_SIZE];       // count the number of mutant-generation trials.
  u32 seed_group_id[MAP_SIZE];       // count the number of mutant-generation trials.
  
  u8 exploit[MAP_SIZE];
  u8 explore[MAP_SIZE];
  u32 group_perf[MAP_SIZE];
  u8 group_delete[MAP_SIZE];
