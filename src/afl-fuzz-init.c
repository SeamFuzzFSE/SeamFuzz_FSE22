...
void setup_seamfuzz_dirs(afl_state_t *afl) {
  ...
  afl->group_common_path_dir = alloc_printf("%s/group_common_path", afl->seam_prob_dir);

  if (access(afl->group_common_path_dir, 0) == -1)
    if (mkdir(afl->group_common_path_dir, 0700)) PFATAL("Unable to create '%s'", afl->group_common_path_dir);
